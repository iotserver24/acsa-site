...
import { CLUB_YEARS, CURRENT_CLUB_YEAR, PREVIOUS_CLUB_YEAR, type ClubYear } from "@/lib/academic-year"
import { TEAM_BY_YEAR, type TeamMember } from "@/lib/team"

export default function TeamPage() {
  const [hoveredMember, setHoveredMember] = useState<TeamMember | null>(null)
  const [year, setYear] = useState<ClubYear>(CURRENT_CLUB_YEAR)
  const teamMembers = TEAM_BY_YEAR[year]
      ...
      <section className="relative py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          <div className="animate-fade-in-up">
            ...
            <div className="mt-8 inline-flex rounded-full border border-cyan-400/30 bg-black/40 p-1">
              {CLUB_YEARS.map((clubYear) => (
                <button
                  key={clubYear}
                  type="button"
                  onClick={() => setYear(clubYear)}
      ...
      <section className="relative pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          {teamMembers.length === 0 ? (
            <div className="max-w-2xl mx-auto text-center glass rounded-2xl p-12 border border-cyan-400/20 bg-black/20">
              <h2 className="text-2xl font-bold text-white mb-3" style={{ fontFamily: "'Orbitron', 'Arial', sans-serif" }}>
                {year} core team coming soon
              </h2>
              <p className="text-gray-300 mb-6">
                The new committee is being finalized. The {PREVIOUS_CLUB_YEAR} team is archived below.
              </p>
              <button
                type="button"
                onClick={() => setYear(PREVIOUS_CLUB_YEAR)}
                className="px-6 py-3 rounded-xl bg-cyan-500 text-black font-semibold hover:bg-cyan-400 transition-colors"
              >
                View {PREVIOUS_CLUB_YEAR} team
              </button>
            </div>
          ) : (
